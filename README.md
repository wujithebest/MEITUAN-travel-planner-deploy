...
![image-20260607214121799](https://raw.githubusercontent.com/wujithebest/MEITUAN-travel-planner-deploy/master/images/image-20260607214121799.png)

图示说明：此处建议插入精准规划模式下“日料店→回家”路线结果，展示左侧对话、中央地图路线和右侧行程概览。

![image-20260607214224714](https://raw.githubusercontent.com/wujithebest/MEITUAN-travel-planner-deploy/master/images/image-20260607214224714.png)
...
![image-20260607214555581](https://raw.githubusercontent.com/wujithebest/MEITUAN-travel-planner-deploy/master/images/image-20260607214555581.png)
...
![image-20260607214743683](https://raw.githubusercontent.com/wujithebest/MEITUAN-travel-planner-deploy/master/images/image-20260607214743683.png)

图示说明：前端首页展示项目名称和游客入口，用户可直接进入路线规划系统；当前登录/注册路径统一重定向至 `/app`。

![image-20260607215346860](https://raw.githubusercontent.com/wujithebest/MEITUAN-travel-planner-deploy/master/images/image-20260607215346860.png)
...
当前成本需要区分动态路线和固定演示路线。动态路线会产生 LLM、博查、高德和天气调用；固定用例点击时直接读取 JSON，运行时不调用 LLM、博查或高德搜索，只有构建阶段刷新高德道路缓存时产生一次性高德调用。具体价格以当前供应商账单和环境配置为准，下面的数值仅作为 v2.0 估算口径，与 4.2 节指标基线对齐。
...
说明：以下数据采用 v2.0 版本历史实测统计取平均值，并采用供应商计费模型中的 DeepSeek 缓存命中率 60% 进行估算。固定快照未参与，博查在该批次中未产生调用；高德/天气/地图费用以实际供应商账单为准，未统一计入。

| 参数 | 取值 | 说明 |
| ------------------------- | -----------: | ---------------------------------------------------------- |
| 单轮平均调用博查 API 次数 | 0 次 | 基于 v2.0 历史实测平均值；博查未触发 |
| 单轮平均调用高德 API 次数 | 19.08 次 | 搜索、地理编码、路线段等，单轮约 4–51 次 |
| 单轮平均调用 LLM 次数 | 1 次 | v2.0 实测中有效请求均只触发 1 次 DeepSeek 调用 |
| 单轮平均输入 tokens | 16095 tokens | 基于 v2.0 实测 traces 中 `deepseek_prompt_tokens` 平均值 |
| 输入缓存命中率 | 60% | 系统提示词较稳定，按供应商缓存计费的保守估算口径 |
| 单轮平均输出 tokens | 1820 tokens | 基于 v2.0 实测 traces 中 `deepseek_completion_tokens` 平均值 |
...
以下为 v2.0 版本指标基线，优先采用线上评测截图数据。固定用例应单独统计缓存命中率、快照完整率和地图道路折线完整率；动态路线应以 SSE 完成事件、`PipelineStats`、日志和用户抽样评测为准。

| 环节 | 指标 | 计算方式 | v2.0 取值 |
|------|------|----------|-----------|
| 输出性能 | 路线生成总耗时 | 从请求进入后端到 SSE 最终完成 | 20–60s |
| 意图理解 | 意图字段识别准确率 | 对比标注的城市、时间、固定 POI、主题、餐饮、预算、交通、排除条件等字段 | 85.19% |
| POI 检索编排 | 单轮对话 POI 需求命中率 | 已满足的目标 POI/类别/活动需求 ÷ 标注目标需求数；支持别名、父子 POI、类别等价 | 78.26% |
| 多轮调整 | 多轮任务执行准确率 | 每一轮是否识别正确操作、完成修改，并保留未要求变动的原路线 | 25% |
| 可执行性 | 路线可行性通过率 | 最终无硬性违规：固定点、时间、交通、距离、餐饮/主题约束等 | 75.00% |
| 结果质量 | 路线合理性评分 | 人工评分，关注绕路折返、顺序、时间容量、可执行性，输出 1–5 分 | 3.68 |
| 输出完整性 | 可执行结果完整率 | 成功返回主 POI、路段、地图数据、备选点、路线摘要的比例 | 90.48% |

> 注：受网络、外部 API 抖动和评测样本影响，以实际效果为准。

成本与迭代投入产出指标继续沿用 4.1 节估算口径：

| 指标名称 | 指标类型 | 计算方式 | 取值/目标 | 说明 |
|----------|----------|----------|-----------|------|
| 单轮成本 | 可观测+估算 | API 成本合计 / 对话轮次 | ¥0.0305 | 与 v2.0 成本估算口径对齐 |
