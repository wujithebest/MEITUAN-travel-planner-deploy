...
| 参数                      |         取值 | 说明                                                       |
| ------------------------- | -----------: | ---------------------------------------------------------- |
| 单轮平均调用博查 API 次数 | 不固定 | 由主题、宏观 POI 数量、理由富化和重试决定 |
| 单轮平均调用高德 API 次数 | 不固定 | 包括搜索、地理编码、天气和路线段请求 |
| 单轮平均调用 LLM 次数 | 不固定 | Step1、理由生成和部分重规划分支可能分别调用 |
| 实际 token 数 | 可观测 | 从 `PipelineStats` 的 prompt/completion usage 读取 |
| 固定用例运行时外部调用 | 0 次 | 直接读取固定 JSON；高德道路折线已预生成 |
| 高德/天气/地图服务费用 | 未统一计入 | 以当前高德、和风天气和部署账号资费为准 |
...
说明：以上固定用例统计仅基于仓库中的六份路线快照估算，不代表当前动态路线表现，也不包含腾讯云、MongoDB Atlas 免费额度以外的基础设施费用和高德、天气接口可能产生的额外商业成本。正式成本分析应直接读取完成事件中的资源统计，并按实际供应商账单替换历史估算值。
